...
describe('defaults', () => {
  ...
  test('should transform response string', () => {
    expect((axios.defaults.transformResponse as AxiosTransformer[])[0]('foo=bar'))
      .toBe('foo=bar')
  })
  ...
  test('should use request config', () => {
    ...
    return getAjaxRequest().then(req => {
      expect(req.url).toBe('http://www.example.com/foo')
    })
  })
  ...
  test('should use default config for custom instance', () => {
    ...
    return getAjaxRequest().then(req => {
      expect(req.requestHeaders[instance.defaults.xsrfHeaderName!])
        .toBe('foobarbaz')
  ...
  test('should use GET headers', () => {
    ...
    return getAjaxRequest().then(req => {
      expect(req.requestHeaders['X-CUSTOM-HEADER'])
        .toBe('foo')
